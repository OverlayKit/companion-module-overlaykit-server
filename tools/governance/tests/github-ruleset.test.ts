...
import type { GitHubObservation, GitHubTrustAnchor, GovernanceRun } from '../src/types.js';
...
function authorizationOptions() {
  const activationPlan = buildGitHubRulesetPlan(contract, governancePlan, manifest, anchor);
...
describe('GitHub ruleset controller', () => {
  it('derives a canonical create payload from the compiled trust anchor', () => {
    const first = buildGitHubRulesetPlan(contract, governancePlan, manifest, anchor);
    const second = buildGitHubRulesetPlan(contract, governancePlan, manifest, anchor);

    expect(canonicalJson(first)).toBe(canonicalJson(second));
    expect(first.authorizationChange).toBe('CHG-0001');
  ...
  it('authorizes only an affirmed attested protected-main execution', () => {
    expect(() => assertGitHubRulesetActivationAuthorization(authorizationOptions())).not.toThrow();
  ...
  it('refuses to update or adopt an existing managed ruleset', () => {
    ...
    expect(() => createGitHubRuleset(activationPlan, anchor, [existing], runner)).toThrow(
      'create-only'
    );
  });
  ...
  it('refuses an unconfigured ruleset that already applies to main', () => {
    ...
    expect(() => createGitHubRuleset(activationPlan, anchor, [existing], runner)).toThrow(
      `already applies to ${anchor.protectedRef}`
    );
  });
  ...
  it('reads the exact protected ref commit from GitHub', () => {
    ...
    const runner: GitHubCommandRunner = {
      run(args) {
        expect(args).toEqual(['api', `repos/${anchor.repository}/git/ref/heads/main`]);
