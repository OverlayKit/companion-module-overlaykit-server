...
import { findRepoRoot, loadContract, validateGitHubEvidence } from '../src/repository.js';
import {
...
function pullRequestRun(): GovernanceRun {
  ...
  return {
    ...
    outcomes: plan.gates.map((gate) => ({
      ...
      producerRef:
        gate.tier === 'enforced' ? `https://github.com/${anchor.repository}/actions/runs/42` : null,
      justification: null,
      ...
      rules: [
        ...
        { type: 'non_fast_forward', parameters: null },
        {
          ...
          parameters: {
            ...
            dismiss_stale_reviews_on_push: anchor.ruleset.dismissStaleReviewsOnPush,
            ...
            required_review_thread_resolution: anchor.ruleset.requireReviewThreadResolution,
          },
        },
        { type: 'required_signatures', parameters: null },
        {
          ...
          parameters: {
            ...
            strict_required_status_checks_policy: anchor.ruleset.strictRequiredStatusChecksPolicy,
          },
        },
      ],
    },
...
describe('GitHub root of trust observer', () => {
  ...
  it('keeps runtime readiness but blocks activation when deferred signed identity is invalid', () => {
    const run = pullRequestRun();
    const observed = evidence(run);
    observed.signatures[0]!.verified = false;
    observed.signatures[0]!.reason = 'unknown_key';

    const observation = verifyGitHubEvidence(plan, manifest, run, observed, anchor, runFileHash);
    ...
    expect(observation.blockers).not.toContain(blocker);
    expect(observation.activationBlockers).toContain(blocker);
  });
  ...
  it('keeps runtime readiness but blocks activation while the GitHub boundary is absent', () => {
    const run = pullRequestRun();
    const observed = evidence(run);
    ...
    expect(observation.ready).toBe(true);
    expect(observation.activationReady).toBe(false);
    expect(observation.blockers).not.toContain(
  ...
  it('requires protected-main evidence to carry an exact provenance attestation', () => {
    ...
    const valid = verifyGitHubEvidence(plan, manifest, run, observed, anchor, runFileHash);
    expect(valid.state).toBe('current');

    observed.attestation.sourceCommit = 'f'.repeat(40);
    const invalid = verifyGitHubEvidence(plan, manifest, run, observed, anchor, runFileHash);
