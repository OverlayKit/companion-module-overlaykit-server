...
export const GITHUB_RULESET_PLAN_SCHEMA_VERSION = 'overlaykit-github-ruleset-plan/v1' as const;
...
export function assertGitHubRulesetActivationAuthorization(options: {
  ...
  invariant(
    options.localRef === anchor.protectedRef && options.localCommit === run.subject.commit,
    'RULESET_PRECONDITION_FAILED',
...
function pullRequestRule(anchor: GitHubTrustAnchor): GitHubRulesetRulePayload {
  ...
  return {
    ...
    parameters: {
      ...
      required_review_thread_resolution: requirements.requireReviewThreadResolution,
    },
  };
}
...
function statusChecksRule(anchor: GitHubTrustAnchor): GitHubRulesetRulePayload {
  ...
  return {
    ...
    parameters: {
      ...
      required_status_checks: [...requirements.requiredStatusChecks].sort().map((context) => ({
        context,
        integration_id: anchor.checkAppId,
      })),
      strict_required_status_checks_policy: requirements.strictRequiredStatusChecksPolicy,
    },
  };
}
...
export function buildGitHubRulesetPayload(anchor: GitHubTrustAnchor): GitHubRulesetCreatePayload {
  return {
    ...
    rules: [...anchor.ruleset.requiredRules].sort().map((type) => rulePayload(type, anchor)),
  };
}
...
export function createGitHubRuleset(
  ...
  invariant(
    activationPlan.payloadHash === canonicalHash(activationPlan.payload) &&
      canonicalJson(activationPlan.payload) === canonicalJson(buildGitHubRulesetPayload(anchor)),
    'RULESET_PLAN_INVALID',
  ...
  invariant(
    !existingRulesets.some((ruleset) => githubRulesetAppliesToRef(ruleset, anchor.protectedRef)),
    'RULESET_PRECONDITION_FAILED',
  ...
  const response = parseJson(
    runner.run(
      ['api', '--method', 'POST', `repos/${anchor.repository}/rulesets`, '--input', '-'],
  ...
  invariant(blockers.length === 0, 'RULESET_ACTIVATION_INVALID', blockers.join(' '));
  return created;
}
