...
export function compileGovernance(contract: LoadedContract): GovernancePlan {
  const { decisionsById, specificationsById, supersededBy, specificationSupersededBy } =
    validateContract(
      contract.decisions,
      contract.profile,
      contract.mechanisms,
  ...
  const specificationLineageIds = collectSpecificationLineage(contract.profile, specificationsById);
  const specificationLineageRecords = [...specificationLineageIds]
    ...
    .sort((left, right) => left.specification.id.localeCompare(right.specification.id));
  const specifications: CompiledSpecification[] = specificationLineageRecords.map((record) => {
  ...
  const profileHash = compileProfileHash(contract, lineageRecords, specificationLineageRecords);
