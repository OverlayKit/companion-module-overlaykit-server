...
export function buildManifest(contract: LoadedContract, plan: GovernancePlan): GovernanceManifest {
  ...
  const specifications = sortedRecord(
    contract.specifications.map((record) => [record.specification.id, record.contentHash] as const)
  );
