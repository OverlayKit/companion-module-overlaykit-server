...
function assertNoSupersessionCycles(decisionsById: Map<string, DecisionRecord>): void {
...
function assertArtifactEnforcement(
  ...
  assertEnforcementBound(artifact.id, artifact.tier, artifact.producedBy, mechanismsById);
}
...
export function validateContract(
  ...
  for (const record of decisions) {
    ...
    invariant(supersedes !== id, 'SUPERSESSION_CYCLE', `${id} cannot supersede itself`);
    invariant(
  ...
  const mechanismsById = indexUnique(registry.mechanisms, 'MECHANISM_DUPLICATE', 'mechanism');
  ...
  for (const id of profile.decisionIds) {
    ...
    invariant(
      record !== undefined,
      'DECISION_NOT_FOUND',
      `Profile references missing decision ${id}`
    );
    invariant(
  ...
  for (const decisionId of profile.decisionIds) {
    ...
    for (const policy of record.decision.policies) {
      if (policy.kind === 'rule') {
        invariant(!ruleIds.has(policy.id), 'RULE_DUPLICATE', `Duplicate rule id: ${policy.id}`);
        ruleIds.add(policy.id);
      }

      if (policy.kind === 'gate') {
        invariant(!gateIds.has(policy.id), 'GATE_DUPLICATE', `Duplicate gate id: ${policy.id}`);
