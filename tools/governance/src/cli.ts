...
import { buildManifest, immutabilityViolations, verifyManifestIntegrity } from './manifest.js';
...
function objectProperty(value: Record<string, unknown>, property: string): Record<string, unknown> {
...
function resolveSubject(repoRoot: string, args: ParsedArgs): EvidenceSubject {
  const commit =
    flag(args, 'commit', process.env.GITHUB_SHA) ?? gitValue(repoRoot, ['rev-parse', 'HEAD']);
  const event = flag(args, 'event', process.env.GITHUB_EVENT_NAME ?? 'local') ?? 'local';
  const ref =
    flag(args, 'ref', process.env.GITHUB_REF) ?? gitValue(repoRoot, ['symbolic-ref', '-q', 'HEAD']);
  const repository =
    flag(args, 'repository', process.env.GITHUB_REPOSITORY) ?? repositoryFromRemote(repoRoot);
...
function recordCommand(repoRoot: string, args: ParsedArgs): void {
  ...
  const run: GovernanceRun = {
    ...
    runId: flag(args, 'run-id') ?? process.env.GITHUB_RUN_ID ?? `local-${Date.now().toString(36)}`,
...
function observeGitHubCommand(repoRoot: string, args: ParsedArgs): void {
  ...
  const observation = verifyGitHubEvidence(plan, manifest, run, evidence, anchor, sha256(rawRun));
  invariant(
    observation.state === 'current' && observation.ready,
    'GITHUB_OBSERVATION_INVALID',
    observation.reason ?? (observation.blockers.join(' ') || 'GitHub evidence is not ready')
  );
...
function verifyGitHubSignaturesCommand(repoRoot: string, args: ParsedArgs): void {
  ...
  const evidence = collectGitHubIdentityEvidence(subject, createGitHubCliRunner(repoRoot));
...
function rulesetPlanCommand(repoRoot: string, args: ParsedArgs): void {
  ...
  const anchor = githubTrustAnchor(plan, flag(args, 'trust-anchor'));
  const activationPlan = buildGitHubRulesetPlan(contract, plan, manifest, anchor);
...
function rulesetApplyCommand(repoRoot: string, args: ParsedArgs): void {
  ...
  const created = createGitHubRuleset(activationPlan, anchor, preEvidence.rulesets.items, runner);
  ...
  writeJson(join(repoRoot, requiredFlag(args, 'observation-out')), postObservation);
  ...
  invariant(
    postObservation.state === 'current' && postObservation.ready && postObservation.activationReady,
