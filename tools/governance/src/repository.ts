...
function assertSchema<T>(
  ...
  invariant(validator(value), 'SCHEMA_INVALID', `${path}: ${formatAjvErrors(validator.errors)}`);
}
...
export function verifyMechanismBindings(repoRoot: string, registry: MechanismRegistry): void {
...
export function readBaseManifest(repoRoot: string, baseRef: string): GovernanceManifest | null {
  try {
    const raw = execFileSync('git', ['show', `${baseRef}:${GOVERNANCE_PATH}/manifest.json`], {
      cwd: repoRoot,
      encoding: 'utf8',
