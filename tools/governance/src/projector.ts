...
function invalid(plan: GovernancePlan, run: GovernanceRun, reason: string): GovernanceObservation {
...
function producerMatchesMechanism(run: GovernanceRun, mechanism: EnforcementMechanism): boolean {
...
export function observeRun(
  ...
  if (actor.kind !== run.invokedBy.kind || actor.principal !== run.invokedBy.principal) {
  ...
  const mechanismIndex = new Map(plan.mechanisms.map((mechanism) => [mechanism.id, mechanism]));
