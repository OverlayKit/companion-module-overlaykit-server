...
function property(value: JsonObject, name: string, label: string): unknown {
  invariant(Object.hasOwn(value, name), 'GITHUB_EVIDENCE_INVALID', `${label} is missing ${name}`);
  return value[name];
}
...
function pagedApi(runner: GitHubCommandRunner, endpoint: string): unknown[] {
  const pages = array(
    parseJson(runner.run(['api', '--paginate', '--slurp', endpoint]), `gh api ${endpoint}`),
    `${endpoint} pages`
  );
...
function normalizeSignature(value: unknown): GitHubSignatureEvidence {
  ...
  return {
    ...
    reason: string(property(verification, 'reason', 'commit verification'), 'verification reason'),
...
function identityEvidenceReasons(
  ...
  if (subject.pullRequest !== null && !signatureCommits.has(subject.pullRequest.headCommit)) {
...
function signatureVerificationBlockers(signatures: GitHubSignatureEvidence[]): string[] {
  return signatures
    .filter((signature) => !signature.verified || signature.reason !== 'valid')
    .map((signature) => `Commit ${signature.commit} is not GitHub-verified (${signature.reason}).`);
}
...
export function assertGitHubIdentityVerified(
  ...
  invariant(blockers.length === 0, 'GITHUB_IDENTITY_INVALID', blockers.join(' '));
}
...
export function normalizeGitHubRuleset(value: unknown): GitHubRulesetEvidence {
  ...
  return {
    ...
    enforcement: string(property(ruleset, 'enforcement', 'ruleset'), 'ruleset enforcement'),
    source: string(property(ruleset, 'source', 'ruleset'), 'ruleset source'),
    sourceType: string(property(ruleset, 'source_type', 'ruleset'), 'ruleset source type'),
    conditions:
...
export function collectGitHubRulesets(
  ...
  return summaries
    ...
    .map((id) => normalizeGitHubRuleset(api(runner, `repos/${anchor.repository}/rulesets/${id}`)))
...
export function collectGitHubEvidence(
  ...
  const attempt = number(property(workflow, 'run_attempt', 'workflow run'), 'workflow run attempt');
  ...
  return {
    ...
    workflow: {
      ...
      workflowPath: string(property(workflow, 'path', 'workflow run'), 'workflow path'),
      ...
      job: {
        ...
        checkName: string(property(checkRun, 'name', 'check run'), 'workflow check name'),
        checkAppId: number(property(checkApp, 'id', 'check app'), 'check app id'),
        checkAppSlug: string(property(checkApp, 'slug', 'check app'), 'check app slug'),
        ...
        runnerEnvironment: runnerGroup === 'GitHub Actions' ? 'github-hosted' : 'self-hosted',
...
export function githubRulesetAppliesToRef(ruleset: GitHubRulesetEvidence, ref: string): boolean {
...
export function githubRulesetActivationBlockers(
  ...
  const managedRulesets = applicable.filter((ruleset) => ruleset.name === requirements.name);
  ...
  if (managed && (managed.source !== anchor.repository || managed.sourceType !== 'Repository')) {
  ...
  for (const context of requirements.requiredStatusChecks) {
    if (
      !statusChecks.some(
        (check) => check.context === context && check.integrationId === anchor.checkAppId
      )
    ) {
      blockers.push(`Required status check ${context} is not bound to ${anchor.checkAppSlug}.`);
    }
  }
  ...
  if (
    !statusCheckRules.some(
      (rule) =>
        ...
        (rule.parameters?.do_not_enforce_on_create ?? false) === requirements.doNotEnforceOnCreate
    )
  ) {
  ...
  const matchingPullRequestRule = pullRequestRules.some((rule) => {
    ...
    return (
      ...
      parameters.required_review_thread_resolution === requirements.requireReviewThreadResolution &&
      parameters.dismiss_stale_reviews_on_push === requirements.dismissStaleReviewsOnPush &&
  ...
  if (
    ...
    !pullRequestRules.some((rule) => rule.parameters?.required_review_thread_resolution === true)
  ) {
...
export function verifyGitHubEvidence(
  ...
  const expectedWorkflowCommit = run.subject.pullRequest?.headCommit ?? run.subject.commit;
  ...
  if (
    (evidence.workflow.conclusion !== null && evidence.workflow.conclusion !== 'success') ||
